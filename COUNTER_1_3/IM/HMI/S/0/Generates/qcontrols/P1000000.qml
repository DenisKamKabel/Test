import QtQuick 2.7
import "qrc:/"
IGuiPage
{
	id: q16777216
	objId: 16777216
	x: 0
	y: 0
	width: 480
	height: 272
	IGuiContainer
	{
		id: q369098752
		objId: 369098752
		x: 0
		y: 0
		width: 480
		height: 272
		z: 1
	}
	IGuiIOField
	{
		id: q33554437
		objId: 33554437
		x: 226
		y: 233
		width: 111
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff31344a"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiTextField
	{
		id: q268435465
		objId: 268435465
		x: 109
		y: 233
		width: 109
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff31344a"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiIOField
	{
		id: q33554438
		objId: 33554438
		x: 202
		y: 84
		width: 255
		height: 54
		qm_Transparent : true 
		qm_TextColor: "#ffff0000"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiTextField
	{
		id: q268435466
		objId: 268435466
		x: 7
		y: 20
		width: 113
		height: 37
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiSymbolicIOField
	{
		id: q335544321
		objId: 335544321
		x: 137
		y: 52
		width: 129
		height: 37
		qm_BorderCornerRadius: 7
		qm_BorderWidth: 4
		qm_ImageSource: "image://QSmartImageProvider/30#2#4#128#0#0"
		qm_Border.top: 9
		qm_Border.bottom: 9
		qm_Border.right: 9
		qm_Border.left: 9
		qm_FillColor: "#ffffff9c"
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentHorizontal: Text.AlignHCenter
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 6
		qm_Anchors.leftMargin: 7
		qm_Anchors.rightMargin: 31
		qm_Anchors.topMargin: 6
		qm_FocusWidth: 0
		qm_FocusColor: "#ff000000"
		qm_GraphicImageId : 6 
		qm_IsDownButtonVisible: true
		qm_SymIOType: 1
		qm_NoOfVisibleRows:11
		qm_HotAreaWidth:25
		qm_SymIOSelBackFillColor: "#ffffff9c"
		qm_SymIOAltBackFillColor: "#ffe7e7ef"
		qm_SymIOSelForeColor:"#ff0000ff"
		qm_NoOfSymIoTextListItems:11
		qm_hasScrollIndicator: false
	}
	IGuiTextField
	{
		id: q268435467
		objId: 268435467
		x: 65
		y: 63
		width: 55
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiLine
	{
		id: q671088649
		objId: 671088649
		qm_LineWidth: 3
		qm_BorderStyle: 0
		qm_TextColor: "#ff0000ff"
		qm_LineStartArrow: false
		qm_LineEndArrow: false
		qm_LineEndsShape: 0
		qm_FillColor: "#ffffffff"
		qm_FillStyle: true
	}
	IGuiTextField
	{
		id: q268435468
		objId: 268435468
		x: 12
		y: 101
		width: 149
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiIOField
	{
		id: q33554439
		objId: 33554439
		x: 202
		y: 177
		width: 174
		height: 41
		qm_BorderCornerRadius: 7
		qm_BorderWidth: 3
		qm_RectangleBorder.width:3
		qm_RectangleBorder.color:"#ff313031"
		qm_FillColor: "#ffffff9c"
		qm_FocusWidth: 0
		qm_FocusColor: "#00000000"
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 5
		qm_Anchors.leftMargin: 6
		qm_Anchors.rightMargin: 5
		qm_Anchors.topMargin: 5
	}
	IGuiTextField
	{
		id: q268435469
		objId: 268435469
		x: 12
		y: 179
		width: 182
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiTextField
	{
		id: q268435470
		objId: 268435470
		x: 218
		y: 5
		width: 76
		height: 30
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiLine
	{
		id: q671088650
		objId: 671088650
		qm_LineWidth: 3
		qm_BorderStyle: 0
		qm_TextColor: "#ff0000ff"
		qm_LineStartArrow: false
		qm_LineEndArrow: false
		qm_LineEndsShape: 0
		qm_FillColor: "#ffffffff"
		qm_FillStyle: true
	}
	IGuiButton
	{
		id: q486539290
		objId: 486539290
		x: 27
		y: 231
		width: 64
		height: 41
		qm_BorderCornerRadius: 7
		qm_BorderWidth: 2
		qm_ImageSource: "image://QSmartImageProvider/21#2#4#128#0#0"
		qm_Border.top: 9
		qm_Border.bottom: 9
		qm_Border.right: 9
		qm_Border.left: 9
		qm_FillColor: "#ffffcf00"
		qm_TextColor: "#ff000000"
		qm_ValueVarTextAlignmentHorizontal: Text.AlignHCenter
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 2
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
		qm_FocusWidth: 2
		qm_FocusColor: "#ff94b6e7"
	}
	IGuiButton
	{
		id: q486539291
		objId: 486539291
		x: 384
		y: 231
		width: 64
		height: 41
		qm_BorderCornerRadius: 7
		qm_BorderWidth: 2
		qm_ImageSource: "image://QSmartImageProvider/21#2#4#128#0#0"
		qm_Border.top: 9
		qm_Border.bottom: 9
		qm_Border.right: 9
		qm_Border.left: 9
		qm_FillColor: "#ffffcf00"
		qm_TextColor: "#ff000000"
		qm_ValueVarTextAlignmentHorizontal: Text.AlignHCenter
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 2
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
		qm_FocusWidth: 2
		qm_FocusColor: "#ff94b6e7"
	}
	IGuiLine
	{
		id: q671088651
		objId: 671088651
		qm_LineWidth: 3
		qm_BorderStyle: 0
		qm_TextColor: "#ff0000ff"
		qm_LineStartArrow: false
		qm_LineEndArrow: false
		qm_LineEndsShape: 0
		qm_FillColor: "#ffffffff"
		qm_FillStyle: true
	}
	IGuiGraphicButton
	{
		id: q486539292
		objId: 486539292
		x: 428
		y: 7
		width: 37
		height: 30
		qm_BorderCornerRadius: 7
		qm_BorderWidth: 2
		qm_ImageSource: "image://QSmartImageProvider/24#2#4#128#0#0"
		qm_Border.top: 9
		qm_Border.bottom: 9
		qm_Border.right: 9
		qm_Border.left: 9
		qm_Transparent : true 
		qm_FocusWidth: 2
		qm_FocusColor: "#ff94b6e7"
		qm_ImagePossitionX: 2
		qm_ImagePossitionY: 2
		qm_ImageWidth: 33
		qm_ImageHeight: 26
		qm_SourceSizeWidth: 33
		qm_SourceSizeHeight: 26
	}
	IGuiTextField
	{
		id: q268435493
		objId: 268435493
		x: 12
		y: 140
		width: 188
		height: 29
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
	IGuiIOField
	{
		id: q33554457
		objId: 33554457
		x: 202
		y: 138
		width: 174
		height: 41
		qm_Transparent : true 
		qm_TextColor: "#ff0000ff"
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_Anchors.bottomMargin: 2
		qm_Anchors.leftMargin: 3
		qm_Anchors.rightMargin: 2
		qm_Anchors.topMargin: 2
	}
}
